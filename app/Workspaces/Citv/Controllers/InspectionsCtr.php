...
class InspectionsCtr extends Controller
{
    ...
    {          
        ...
        $token = $instace->options['dinamics']["serviceCITVLastOpen"];
        if( !CoordinatorMTCSoap::isValidToken( $token )  ){
            $creds = $instace->options["serviceCITVcreds"];
            $newToken = CoordinatorMTCSoap::refreshToken( $creds );
            $newToken["date"] = WorkspaceDateHelper::now(); 
            $newToken["user"] = "Frank V. ";
            
            $instace->options['dinamics']["serviceCITVLastOpen"] = $newToken;
            
            \Workspace::instanceUpdate(   $instace );   
        } 
            
         $viewData['serviceCITVLastOpen'] =   $instace->options['dinamics']["serviceCITVLastOpen"];
         $inspections = InspectionMdl::where('facility_id', $instace->facilityId)
                                        ->whereNotIn('inspection_status', ['terminado', 'cancelado'])
                                        ->get(); 
 
         return view('citv::inspections.inspections', ['inspections' => $inspections, 'viewData'=> $viewData]);
    }
    ...
    {   $action = $request->input('action');
        ...
        switch ($action){
            case 'saveInsS1': InspectionActionsCtr::saveInspectionStep1( $request ); break;
        }dd( $action);
        /*Valida  los campos
